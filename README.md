...
```yaml
- id: nav
  uses: JevForge/jev-model-navigator@v0.4.1
  env:
...
Pin `@v0.4.1` for reproducibility, or `@v0` for the floating major line.
...
```yaml
uses: JevForge/jev-model-navigator@v0.4.1   # recommended pin
uses: JevForge/jev-model-navigator@v0       # floating major (v0.x)
```

### Cutting a release (CI)

You do **not** need to create the GitHub Release by hand:

1. Merge to `main` with `dist/` up to date (`npm run build`).
2. Either:
   - **Actions → Release → Run workflow** on `main`, enter `0.4.1`, or
   - `git tag v0.4.1 && git push origin v0.4.1`
3. CI runs tests, publishes the GitHub Release, and moves the floating major tag (`v0`).

Marketplace listing updates still need one browser step (GitHub 2FA): open the release and keep **Publish this Action to the GitHub Marketplace** checked.
